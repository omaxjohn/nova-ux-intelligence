...
Deliver the decision/change, concise causal rationale, highest-priority next actions, confidence and actual verification status. Separate executed/passed, failed, planned and blocked checks. Keep minor polish secondary to task failures. For kernel maintenance, use the frozen contract and regression protocol in the canonical repository tests; do not change semantics without new failing-test evidence.
